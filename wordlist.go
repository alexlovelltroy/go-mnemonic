...
// GetRandomName returns a string made up of the number of words requested and a seperator
func GetRandomName(length int, separator string) string {
	var name string
	name = wordlist[rand.Intn(len(wordlist))]
	if length == 1 {
		return name
	}
	for i := 0; i < length-1; i++ {
		name += separator + wordlist[rand.Intn(len(wordlist))]
	}
	return name
}
